...
let {Cc,Ci,Cu,Cm} = require("chrome");

// Dynamically register native XPCOM component
let manifestPath = "chrome.manifest";
let file = Cc["@mozilla.org/file/local;1"].createInstance(Ci.nsILocalFile);
  try {
    file.initWithPath(
        require("url").toFilename(require("self").data.url(manifestPath))
      );
  } catch(e) {
    throw new Error("This file path is not valid : "+cm+"\n"+e);
  }
...
for(var i in subprocess)
  exports[i]=subprocess[i];
